...
| [`skills/building-block-search`](skills/building-block-search/) | Structured literature search as **building-block search**: maximalist concept blocks (population / phenomenon / context pegs), reduction by combining blocks — never by thinning them, pruning only on measured zero unique recall. Includes database-mechanics checklist and search-foundation deliverable format. |
...
ln -s ../agent-skills/skills/building-block-search ~/.agents/skills/building-block-search
```
